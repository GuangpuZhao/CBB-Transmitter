...
/* 	FILE *f=fopen("test-DataCSDbeforeIFFT.txt","w");
	for(i=0;i<N_STS;i++)
	{
		...
		printStreamToFile_float(he_sym[i], 3840, f);
	}
 	 fclose(f);	
...
/*	FILE* fp1=fopen("stf_add_window.txt","w");
	FILE* fp2=fopen("ltf_add_window.txt","w");
	FILE* fp3=fopen("sig_add_window.txt","w");
	for(i=0;i<N_TX;i++){
		printStreamToFile_float(oneStreamOfSTF[i], 320,fp1);
		printStreamToFile_float(oneStreamOfLTF[i], 320,fp2);
		printStreamToFile_float(oneStreamOfSig[i], 160,fp3);
	}
	fclose(fp1);
	fclose(fp2);
	fclose(fp3);*/
	...
	for(i=0;i<N_TX;i++){
	  ...
	  FILE* fp=fopen("HeLTFafterCSD.txt","w");
	for(i=0;i<N_TX;i++)
		printStreamToFile(heLTF[i],n_ltf*256,fp);
	fclose(fp);
	
	complex32* IDFT_X_VHTLTF[N_TX];
	for(i=0;i<N_TX;i++){
		IDFT_X_VHTLTF[i]=(complex32*)malloc(n_ltf*512*sizeof(complex32));
		MKSUREENMEM(IDFT_X_VHTLTF[i]);
		memset(IDFT_X_VHTLTF[i],0,n_ltf*512*sizeof(complex32));
	}
	He_LTF_IDFT(heLTF, IDFT_X_VHTLTF);
	//for(i=0;i<N_TX;i++) free(heLTF[i]);
	 fp=fopen("HeLTFafterIDFT.txt","w");
	for(i=0;i<N_TX;i++)
		printStreamToFile(IDFT_X_VHTLTF[i],n_ltf*512,fp);
	fclose(fp);
	
	const int CpLen = CpDuration*SampRate;
	const int SymbLen = SymbDuration*SampRate;
	//complex32 *X_VHTLTF[N_TX];
	for(i=0; i<N_TX; i++){
		X_VHTLTF[i] = (complex32*)malloc(n_ltf*(SymbLen+CpLen)*sizeof(complex32));
		MKSUREENMEM(X_VHTLTF[i]);
		memset(X_VHTLTF[i],0,n_ltf*(SymbLen+CpLen)*sizeof(complex32));
	}
	addCPforHELTF(X_VHTLTF, IDFT_X_VHTLTF);
	for(i=0;i<N_TX;i++) free(IDFT_X_VHTLTF[i]);
